...
Capability là một kiểu pattern trong ngôn ngữ lập trình Move. Nó giúp kiểm soát việc ai được phép sử dụng các objects trong chương trình. Lấy ví dụ mình muốn tạo một quyền teacher object mà chỉ có object đó mới có quyền quyền thay đổi bảng điểm học sinh. . Khi muốn tạo một capability object mà không thể chuyển cho người khác( giống như soulbound token) , chúng ta chỉ cần bỏ qua thuộc tính store trong struct.
...
Chúng ta truyền tham số với reference của object TeacherCAp đó và dùng underscore(_) để ký hiệu là tham số này không được sử dụng. Nhớ là bạn chỉ đang tham chiếu đó tới object. Như vậy mọi người đã hiểu tầm quan trọng của việc dùng Capability rất quan trọng cho quyền access control với các object. Pattern này sẽ cung cấp một cách flexible hợn. Nếu bạn vẫn chưa hiểu Capability là gì ? 
...
Sau đó nhớ switch sang address khac: 
